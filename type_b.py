...
def generate_type_b():
    ...
    for i in range(4):
        ...
        while curr in ip_octets:
            curr = random.randint(1, 256)
        ip_octets.append(curr)

    ans_3, ans_4 = ip_octets[-2] & mask_3, ip_octets[-1] & mask_4

    ans = [ans_3, ans_4, ip_octets[0], ip_octets[1]]
    common.randomize_list(ans, 1, 255, 4, 10)
    c_ans = [""] * 4

    for i in range(len(ans)):
        if ans[i] == ip_octets[0]:
            c_ans[0] = chr(ord("A")+i)
        elif ans[i] == ip_octets[1]:
            c_ans[1] = chr(ord("A")+i)
        elif ans[i] == ans_3:
            c_ans[2] = chr(ord("A")+i)
        elif ans[i] == ans_4:
    ...
    ip_mask = "IP-адрес: " + common.list2string(ip_octets) + "\n" + "Маска: "\
             + common.list2string([255, 255, mask_3, mask_4])
    task_text = padding1 + ip_mask + "\n" + padding2 + "\n" + out_table.get_str()
    return common.Task(task_text, "".join(c_ans))
